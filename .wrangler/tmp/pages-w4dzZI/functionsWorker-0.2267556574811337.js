...
// api/subscribe.js
async function neonSql2(dbUrl, query, params = []) {
  const u = new URL(dbUrl);
  const host = u.hostname;
  const connStr = dbUrl.split("?")[0] + "?sslmode=require";
  const res = await fetch(`https://${host}/sql`, {
...
// api/talent.js
async function neonSql3(dbUrl, query, params = []) {
  const u = new URL(dbUrl);
  const host = u.hostname;
  const connStr = dbUrl.split("?")[0] + "?sslmode=require";
  const res = await fetch(`https://${host}/sql`, {
...
// ../.wrangler/tmp/pages-w4dzZI/functionsRoutes-0.4831172188827134.mjs
var routes = [
  {
    routePath: "/api/debug",
    mountPath: "/api",
